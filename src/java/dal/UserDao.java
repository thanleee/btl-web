...
public class UserDao extends DBContext{
	...
	public boolean themTaiKhoan(Object obj) {
		User u = (User) obj ; 
                String sql ="insert into sign_up (phone,email,password,cfpassword,fname,lname,mid) values('"+u.getPhone()+"','"+u.getEmail()+"','"+u.getPassword()+"','"+u.getCfpassword()+"','"+u.getFname()+"','"+u.getLname()+"','"+u.getMid()+"'+'"+0+"')";
		try {
	...
	public User layThongTinTaiKhoan(String email) {
		...
		try {   
			...
			while(rs.next()) {
				...
				return new User(phone,email1,password,cfpassword,fname,lname,0) ; 
